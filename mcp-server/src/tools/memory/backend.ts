...
export class MemoryMcpBridge {
  ...
  async start(): Promise<void> {
    ...
    const entry = this.resolveMemoryServerEntry();
    logger.info("Starting prebuilt memory MCP server.", { entry, memoryFilePath: this.memoryFilePath });
    ...
    this.process.stderr.on("data", (chunk: Buffer) => {
      logger.debug("memory-server stderr", { message: chunk.toString("utf8").trim() });
    });
    this.process.on("close", (code: number | null, signal: NodeJS.Signals | null) => {
      const reason = new Error(`Memory MCP process closed (code=${String(code)} signal=${String(signal)}).`);
    ...
    logger.info("Prebuilt memory MCP server initialized.");
  }
  ...
  private resolveMemoryServerEntry(): string {
    let packageJsonPath: string;
    try {
      packageJsonPath = require.resolve("@modelcontextprotocol/server-memory/package.json");
    } catch (_error) {
      const resolvedEntrypoint = require.resolve("@modelcontextprotocol/server-memory");
    ...
    if (!binField) {
      throw new Error("Could not resolve memory server bin from package metadata.");
    }
    const binRelativePath = typeof binField === "string" ? binField : Object.values(binField)[0];
    if (!binRelativePath) {
      throw new Error("Memory server bin field is empty.");
    }
  ...
  private findNearestPackageJson(startDir: string): string {
    ...
    while (true) {
      ...
      if (parent === current) {
        throw new Error("Could not locate package.json for memory MCP server package.");
      }
      current = parent;
    }
  }
  ...
  private async request(method: string, params: JsonObject, timeoutMs?: number): Promise<JsonValue> {
    if (!this.process) {
      throw new Error("Memory MCP process is not started.");
    }
  ...
  private drainJsonLineMessages(): void {
    while (true) {
      ...
      if (!(line.startsWith("{") && line.endsWith("}"))) {
        logger.debug("Ignoring non-JSON stdout line from memory server.", { line });
        continue;
      }
